...
print(variant_type, chromosome, ABC_PATH, GENE_POS, ANNOTATION_FILE_PATH)
...
def make_coding_numerical(df):
    ...
    for val in df['RegulomeDB_score'].unique():
        ...
        if match:
            new_val = float(match.group())
            df.loc[df["RegulomeDB_score"] == val, "RegulomeDB_score"] = new_val
        else:
            print(f"Warning: No numerical value found in 'RegulomeDB_score' for {val}.")
            # Handle this case, e.g., by setting a default value or skipping
            df.loc[df["RegulomeDB_score"] == val, "RegulomeDB_score"] = 0  # Set NaN or a default value
...
def main():
    if variant_type == "coding": 
        annotations = coding_annotations
        to_extract = make_extract_coding()
    if variant_type == "noncoding": 
        annotations = noncoding_annotations
        to_extract = make_extract_noncoding()

    output = os.path.join(OUTPUT_DIR, f"WGSA_chr{chromosome}.csv")
    ...
    if variant_type == "coding":
        ...
        coding_anno.drop_duplicates().to_csv(output)
        ...
        noncoding_anno.drop_duplicates().to_csv(output)
        print("done with ", output)
    return
